#!/usr/bin/env node
// Reproducible WASM build for tree-sitter-rust.
...
const work = await mkdtemp(path.join(tmpdir(), "grammar-rust-build-"));
await run("git", ["clone", "--no-checkout", "https://github.com/tree-sitter/tree-sitter-rust.git", "src"], { cwd: work });
...
// Locate produced wasm and copy as rust.wasm.
const fs = await import("node:fs/promises");
const built = (await fs.readdir(buildCwd)).find((f) => f.endsWith(".wasm"));
if (!built) throw new Error("no .wasm produced");
await copyFile(path.join(buildCwd, built), path.join(repoRoot, "rust.wasm"));
const bytes = (await readFile(path.join(repoRoot, "rust.wasm"))).length;
console.log(`rust.wasm: ${bytes} bytes (built from ${pin})`);
