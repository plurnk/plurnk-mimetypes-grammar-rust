#!/usr/bin/env node
// CI byte-identical reproducibility check for rust.wasm.
...
const committed = await readFile(path.join(repoRoot, "rust.wasm"));
const committedHash = createHash("sha256").update(committed).digest("hex");
console.log(`committed rust.wasm sha256: ${committedHash}`);

const work = await mkdtemp(path.join(tmpdir(), "grammar-rust-verify-"));
...
console.log(`rebuilt rust.wasm sha256: ${rebuiltHash}`);
